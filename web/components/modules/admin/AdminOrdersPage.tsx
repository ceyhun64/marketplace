...
export default function AdminOrdersPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get<PaginatedOrders>(`/api/orders/admin/all?${params}`);
      return res.data;
    },
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Platform genelindeki tüm siparişler</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Toplam Sipariş", value: stats.total, icon: ShoppingCart, color: "text-blue-600", bg: "bg-blue-50" },
          { label: "Bekleyen", value: stats.pending, icon: Clock, color: "text-yellow-600", bg: "bg-yellow-50" },
          { label: "Taşımada", value: stats.inTransit, icon: Truck, color: "text-indigo-600", bg: "bg-indigo-50" },
          { label: "Teslim Edildi", value: stats.delivered, icon: CheckCircle, color: "text-green-600", bg: "bg-green-50" },
        ].map((s) => (
          <Card key={s.label}>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50">
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Sipariş ID</TableHead>
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Müşteri</TableHead>
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Mağaza</TableHead>
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Kaynak</TableHead>
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Tutar</TableHead>
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Durum</TableHead>
              <TableHead className="font-semibold text-xs uppercase tracking-wide">Tarih</TableHead>
              <TableHead />
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              filtered.map((order) => (
                <TableRow key={order.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-sm text-gray-600">{order.merchantStoreName ?? "—"}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className="text-xs">
                      {order.source === "MARKETPLACE" ? "Pazaryeri" : "E-Mağaza"}
                    </Badge>
                  </TableCell>
                  <TableCell className="font-semibold text-sm">{formatCurrency(order.totalAmount)}</TableCell>
                  <TableCell>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${ORDER_STATUS_COLORS[order.status] ?? "bg-gray-100 text-gray-800"}`}>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2">
          ...
          <span className="px-3 py-1 text-sm text-gray-600">Sayfa {page} / {totalPages}</span>
      ...
      <Dialog open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Sipariş #{selectedOrder?.id.slice(0, 8).toUpperCase()}</DialogTitle>
          </DialogHeader>
          {selectedOrder && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedOrder.merchantStoreName ?? "—"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p>{selectedOrder.shippingRate === "EXPRESS" ? "⚡ Ekspres" : "📦 Standart"}</p>
                </div>
                {selectedOrder.shipment?.trackingNumber && (
                  <div className="col-span-2">
                    <p className="text-xs text-gray-500 mb-0.5">Takip No</p>
                    <p className="font-mono text-sm">{selectedOrder.shipment.trackingNumber}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${ORDER_STATUS_COLORS[selectedOrder.status] ?? ""}`}>
                  ...
                  {NEXT_STATUSES[selectedOrder.status] && (
                    <>
                      ...
                      <Button
                        size="sm"
                        onClick={() => updateStatusMutation.mutate({ id: selectedOrder.id, status: NEXT_STATUSES[selectedOrder.status]! })}
