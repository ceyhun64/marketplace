...
function CategoryCard({ category }: { category: Category }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-black/5 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      ...
      >
        {/* Icon / colour swatch */}
        <div className="w-12 h-12 rounded-xl  flex items-center justify-center flex-shrink-0 group-hover:bg-[var(--charcoal)] transition-colors">
          {category.iconUrl ? (
            <img
            ...
            <Tag className="w-5 h-5 text-[var(--charcoal)]/40 group-hover:text-white transition-colors" />
          )}
        </div>
        ...
        <ChevronRight className="w-4 h-4 text-[var(--charcoal-soft)] group-hover:text-[var(--red)] transition-colors flex-shrink-0" />
      </Link>
      ...
      {hasChildren && (
        <>
          <button
            onClick={() => setOpen((p) => !p)}
            className="w-full text-left px-5 pb-3 text-[11px] font-mono uppercase tracking-widest text-[var(--charcoal-soft)] hover:text-[var(--charcoal-mid)] transition-colors"
          >
              ...
              {category.subCategories!.map((sub) => (
                <li key={sub.id}>
                  <Link
                    ...
                    className="flex items-center justify-between px-5 py-3 text-[13px] text-[var(--charcoal)]/70 hover:text-[var(--red)] hover: transition-colors"
                  >
    ...
    <main className="min-h-screen ">
      ...
      <div className="max-w-325 mx-auto px-4 lg:px-8 py-12">
        ...
        {!isLoading && !isError && rootCategories.length === 0 && (
          <div className="text-center py-20 text-[var(--charcoal-soft)]">
